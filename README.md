...
7. 清理其他临时文件：
   ...
   ```
8. 运行自检清单
   自检之前，一定要先读一下 README 里的元数据规范，然后依次检查：
   - 文件名是否是有意义的英文名
   - 再次读取一下元数据，看看格式是否合乎规范
   - 先读取预定义的 Tag 列表，然后检查 Tag 是否符合规范
   - 日期是否用了原文件的里的创建日期
   - 是否删除了压缩图片后的原始图片
   - 逐个检查，是否清理了所有的临时文件
   - 是否还有其他漏掉的步骤
...
1. **获取视频 ID**：
   ...
   * 例如：`https://youtu.be/cKONu4-p0ws?si=xHBSDw4KvG0dbuC7` 中的 ID 是 `cKONu4-p0ws`
   * 或者：`https://www.youtube.com/watch?v=cKONu4-p0ws` 中的 ID 是 `cKONu4-p0ws`
...
3. **最佳实践**：
   * 在视频前添加相关的文字说明
   * 确保视频内容与文章主题相关
   * 考虑到一些地区可能无法访问 YouTube，建议在视频附近添加截图或关键信息的文字描述

4. **性能考虑**：
   * YouTube 视频嵌入会增加页面加载时间
   * 建议一篇文章中控制视频数量
   * 如果需要展示多个视频，考虑使用视频截图+链接的形式
...
适用于外部链接（如豆瓣、Notion 等），使用 `link` shortcode：
...
适用于内部链接（如文章之间的相互引用），使用标准 Markdown 语法：
