...
博客使用了 PaperMod 主题。从 2024-02-20 起，主题代码已经从 git submodule 转换为普通文件，以简化管理和定制，构建属于自己的极简风格。

#### 主题管理方式的变更
* **变更前**：通过 git submodule 引入 PaperMod 主题
* **变更后**：主题文件直接包含在项目中
* **变更原因**：
  * 简化主题管理，不再需要处理 git submodule
  * 方便直接修改主题文件，无需通过覆盖方式定制
  * 更好的版本控制，所有修改都记录在项目历史中
  * 避免因上游主题更新导致的潜在问题

#### 主题定制说明
* 主题文件位于 `themes/PaperMod` 目录
* 可以直接编辑主题文件进行定制
* 建议在修改主题文件前先在此文档中记录修改内容
* 重要修改应该提交单独的 commit，并附带清晰的说明

#### 注意事项
* 此方式意味着无法自动获取 PaperMod 主题的更新
* 如果将来需要更新主题，需要手动对比和合并更改
* 建议在修改主题文件之前先备份
...
1. **选择合适的样式**：
   - 如果链接是文章的主要推荐内容，使用 Bookmark 样式
   - 如果链接是列表项的一部分或行内引用，使用普通链接
   - 在同一个列表中保持一致的链接样式
...
4. **特殊情况处理**：
   - 如果列表项需要特别强调某个链接，可以将该链接单独成段并使用 Bookmark 样式
   - 对于参考文献类列表，统一使用普通链接样式
