...
> 💡 这一步仅 Mac 用户需要操作，iPhone 和 iPad 跳过即可

Mac 版 Shadowrocket 打开后，默认只在 `127.0.0.1:1082` 提供一个 HTTP 代理端口，不会接管系统流量。这意味着浏览器访问一般没有问题，但终端（Terminal）、Claude Code 等非浏览器的命令行工具，就不会走代理，会提示区域不支持之类的错误。

解决方法很简单：打开 **设置 - 隧道**，开启「强制路由」即可，其他选项（包括所有网络、包括本地网络、包括 APNs、包括蜂窝网络）都不需要动。
